...
+ (void)resetPreferences {
    ...
    [defaults removeObjectForKey: @"DefaultsTagAngBibliaRemoved"];
...
//	DLog(@"start the Launch configuring...");
    ...
    //additional bibles
    BOOL TagAngBiblia =[defaults boolForKey:@"TagAngBiblia"];
   ...
   // BOOL PorCapNT=[defaults boolForKey:@"PorCapNT"];
    ...
    {
        [defaults synchronize];
        SwordModule *TagAngBibliaModule = [[moduleManager swordManager] moduleWithName:@"TagAngBiblia"];
        if(TagAngBibliaModule) {
            // if it's already installed, remove the search index because it will now be out of date
            [TagAngBibliaModule deleteSearchIndex];
        }
        [moduleManager installModulesFromZip: [[NSBundle mainBundle] pathForResource:@"TagAngBiblia" ofType:@"zip"] ofType: bible removeZip:NO internalModule:YES];
        
        [defaults setBool: YES forKey:@"TagAngBiblia"];
	...
	BOOL tagAngBibliaModule = [[moduleManager swordManager] isModuleInstalled:@"TagAngBiblia"];
	BOOL tagAngBibliaModuleRemoved = [defaults boolForKey:@"DefaultsTagAngBibliaRemoved"];
    ...
    if(!tagAngBibliaModule  && !tagAngBibliaModuleRemoved) {
        //reinstall the kjv module!
        DLog(@"reinstalling tagAngBibliaModuleRemoved");
        [moduleManager installModulesFromZip: [[NSBundle mainBundle] pathForResource:@"TagAngBiblia" ofType:@"zip"] ofType: bible removeZip:NO internalModule:YES];
    }
